...
export function buildMessage(trip) {
  ...
  const lines = [
    ...
    "",
    name?.trim()
      ? `Je m'appelle ${name.trim()} et je souhaite organiser un voyage sur-mesure à Bali avec vous.`
  ...
  /* Le mot libre passe **après** les cases cochées et avant la demande
     de devis : il commente ce qui précède, et c'est la dernière chose
     lue avant la question. Le texte est repris tel quel, sauts de ligne
     compris — le reformater serait réécrire ce que le voyageur a voulu
     dire. */
  const mot = note?.trim();
  if (mot) {
    lines.push("", "✍️ Ce que je voulais vous dire :", mot);
  }
...
export function buildSubject(trip) {
  /* Nom complet depuis qu'on le demande : c'est ce qu'Agus retrouvera
     en cherchant dans sa boîte six semaines plus tard, et deux Camille
     dans la même saison n'ont rien d'improbable. */
  const qui = trip.name?.trim() ? `${trip.name.trim()} — ` : "";
